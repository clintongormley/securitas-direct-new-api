...
class TestGetAirQualityData:
    ...
    async def test_returns_none_value_when_no_hours(self, client, transport):
        """Returns AirQuality with value=None when hours list is empty."""
        transport.execute.return_value = air_quality_response(
            data={
                "status": {"current": 85},
                "hours": [],
    ...
    async def test_returns_none_value_when_hours_null(self, client, transport):
        """Returns AirQuality with value=None when hours is null."""
        transport.execute.return_value = air_quality_response(
            data={
                "status": {"current": 2},
                "hours": None,
