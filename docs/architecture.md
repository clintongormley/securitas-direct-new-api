...
- **Session sharing** — Multiple config entries for the same username share a single `VerisureHub` instance, tracked in `hass.data[DOMAIN]["sessions"]`. This prevents duplicate logins and reduces WAF pressure. Each record carries `holders`, the set of entry ids currently using the hub — that set is the whole of the bookkeeping, so how many entries are using a hub is `len(holders)` and never a separate tally that could drift. The hub is dropped once `holders` empties, either when the last entry unloads (`_release_shared_session`) or when an abandoned config flow cleans up after itself (`_cleanup_flow_session`). Holding entry ids rather than keeping a count makes a reference idempotent per entry, so a setup Home Assistant retries after `ConfigEntryNotReady` — which it does without unloading first — still counts once, and an entry holding no reference cannot take a co-tenant's reference away. The config flow builds its hub before its entry exists, so that record starts with no holders and the entry adopts it during setup.
