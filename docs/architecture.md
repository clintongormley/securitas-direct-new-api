...
- **Session sharing** — Multiple config entries for the same username share a single `VerisureHub` instance, tracked in `hass.data[DOMAIN]["sessions"]`. This prevents duplicate logins and reduces WAF pressure. Each session record holds `holders`, the set of entry ids currently using the hub, and `ref_count`, which is always `len(holders)`. Counting entry ids rather than keeping a bare tally makes a reference idempotent per entry: Home Assistant retries a setup that raised `ConfigEntryNotReady` without unloading first, so the same entry can run `_get_or_create_session` repeatedly and still count once. Releasing on behalf of an entry that never completed setup changes nothing, so it cannot pop the session out from under a co-tenant. The hub is dropped when `holders` empties. The config flow registers its hub before the entry exists, so that record starts with no holders and a count of zero, and the entry adopts it during setup.
