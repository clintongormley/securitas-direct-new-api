...
- **Session sharing** — Multiple config entries for the same username share a single `VerisureHub` instance, tracked in `hass.data[DOMAIN]["sessions"]`. This prevents duplicate logins and reduces WAF pressure. Each record holds `holders`, the set of entry ids using the hub, and `ref_count`, which is always `len(holders)`; the hub is dropped once `holders` empties. Counting entry ids rather than keeping a tally makes a reference idempotent per entry, so a setup Home Assistant retries after `ConfigEntryNotReady` — which it does without unloading first — still counts once, and releasing for an entry that holds nothing cannot take a co-tenant's reference away. The config flow builds its hub before its entry exists, so that record starts with no holders and the entry adopts it during setup.
